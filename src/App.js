...
const fallbackCheckoutProviderByMode = {
  mock: 'mock',
  paypal: 'paypal',
  postepay: BANK_TRANSFER_PROVIDER,
  bank_transfer: BANK_TRANSFER_PROVIDER,
  hybrid: 'paypal',
};
const FALLBACK_CHECKOUT_PROVIDER = fallbackCheckoutProviderByMode[FALLBACK_PAYMENT_MODE] || 'mock';
...
const BANK_TRANSFER_DETAILS = {
  beneficiary: 'RAZZA ELENA',
  iban: 'IT81I3608105138284162484165',
};
...
function resolvePaymentProviderBadge(provider) {
  ...
  if (provider === BANK_TRANSFER_PROVIDER) {
    return 'Bonifico';
  }
  return 'PayPal';
}

function resolvePaymentProviderTitle(provider) {
  if (provider === 'mock') {
    return 'Pagamento simulato (ambiente test)';
  }
  if (provider === BANK_TRANSFER_PROVIDER) {
    return 'Metodo di pagamento: Bonifico';
  }
  return 'Paga in sicurezza con PayPal';
}

function resolvePaymentProviderDescription(provider) {
  if (provider === 'mock') {
    return 'Conferma immediata per ambiente test.';
  }
  if (provider === BANK_TRANSFER_PROVIDER) {
    return 'Pagamento manuale con verifica bonifico.';
  }
  return 'Checkout online protetto con PayPal.';
}

function normalizePaymentProvider(providerRaw) {
  const provider = String(providerRaw || '').trim().toLowerCase();
  if (provider === 'postepay' || provider === 'bonifico' || provider === BANK_TRANSFER_PROVIDER) {
    return BANK_TRANSFER_PROVIDER;
  }
  if (provider === 'paypal' || provider === 'mock') {
    return provider;
  }
  return '';
}
...
function App() {
  ...
  const [, setPaymentMode] = useState(FALLBACK_PAYMENT_MODE);
  ...
  const paymentActionsRef = useRef(null);
  const bankTransferPanelRef = useRef(null);
  ...
  const scrollPaymentDetailsIntoView = useCallback((provider) => {
    if (typeof window === 'undefined') {
      return;
    }
      ...
      window.requestAnimationFrame(() => {
        const targetElement =
          provider === BANK_TRANSFER_PROVIDER
            ? bankTransferPanelRef.current
            : paymentActionsRef.current;
  ...
  const visiblePaymentProviders = useMemo(
    () => paymentProviders.filter((provider) => provider !== 'mock'),
    [paymentProviders]
  );
  const bookingTotalAmount = selectedTour ? selectedTour.price * Number(people) : 0;
  ...
  const isBankTransferConfirmation = currentStep === 7 && selectedPaymentProvider === BANK_TRANSFER_PROVIDER && bankTransferAcknowledged;
  ...
  useEffect(() => {
    if (currentStep !== 6 || visiblePaymentProviders.length === 0) {
      return;
    }
    if (visiblePaymentProviders.includes(selectedPaymentProvider)) {
      return;
    }
    setSelectedPaymentProvider(visiblePaymentProviders[0]);
    setPaypalSdkError('');
      ...
      try {
        ...
        const providers = Array.isArray(payload.providers)
          ? payload.providers
            ...
            .filter((provider) => SUPPORTED_PAYMENT_PROVIDERS.includes(provider))
          : [];
        const normalizedProviders = Array.from(
          new Set([...(providers.length > 0 ? providers : [FALLBACK_CHECKOUT_PROVIDER]), BANK_TRANSFER_PROVIDER])
        );
        ...
        if (!cancelled) {
          ...
          setPaymentProviders(Array.from(new Set([FALLBACK_CHECKOUT_PROVIDER, BANK_TRANSFER_PROVIDER])));
  ...
  const goToPaymentStep = () => {
    ...
    setSelectedPaymentProvider((current) => (
      visiblePaymentProviders.includes(current)
        ? current
        : (visiblePaymentProviders[0] || paymentProviders[0] || FALLBACK_CHECKOUT_PROVIDER)
    ));
  ...
  const handlePaymentSuccess = useCallback(async ({ confirmPayload, paymentProvider, paymentReference }) => {
    ...
    const normalizedBookingStatus = String(confirmPayload.bookingStatus || '').trim().toLowerCase();
    const isPendingBankTransfer =
      normalizedPaymentProvider === BANK_TRANSFER_PROVIDER || normalizedBookingStatus === 'pending';
    setBankTransferAcknowledged(isPendingBankTransfer);
    const totalAmountEur = Number(
      confirmPayload.totalPriceEur
      ?? (selectedTour ? selectedTour.price * Number(people) : 0)
    );
    setCompletedBooking({
      ...
      confirmedAt: isPendingBankTransfer
        ? ''
  ...
  const completeManualPayment = useCallback(async ({ provider, reference }) => {
    const normalizedProvider = String(provider || '').trim().toLowerCase();
    const normalizedReference = sanitizeCustomerText(reference || '', 128);
    const paymentReference = normalizedReference || `${normalizedProvider.toUpperCase()}_${Date.now()}`;
  ...
  const startCheckout = () => {
    ...
    window.alert(translateText('Metodo di pagamento non supportato.'));
  };
        ...
        <div className="hero-content hero-layout">
          ...
          <div className="hero-copy">
            <h1>
              ...
              <br />
              <span className="hero-title-light">Tuk Tuk</span>
              <br />
          ...
          {currentStep === 6 ? (
            <div className="booking-stage-panel">
              <div className="booking-payment-card">
                ...
                <div className={`booking-payment-methods ${visiblePaymentProviders.length === 1 ? 'single' : ''}`}>
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setPaypalSdkError('');
                          setBankTransferAcknowledged(false);
                          scrollPaymentDetailsIntoView(provider);
                        }}
                        disabled={isPaying}
                      >
                        <span className="pp-badge">{resolvePaymentProviderBadge(provider)}</span>
                        <strong>{resolvePaymentProviderTitle(provider)}</strong>
                        <small>{resolvePaymentProviderDescription(provider)}</small>
                        <small>
                          Totale: EUR {formatEurAmount(bookingTotalAmount, locale)}
                        </small>
                      </button>
                    );
                  })}
                </div>
                {selectedPaymentProvider === BANK_TRANSFER_PROVIDER ? (
                  <div ref={bankTransferPanelRef} className="booking-bank-transfer-panel">
                    <p className="booking-bank-transfer-method">Metodo di pagamento: Bonifico</p>
                    <h4>Completa il pagamento tramite bonifico</h4>
                    <p className="booking-bank-transfer-description">
                      Per confermare la prenotazione, effettua il pagamento tramite bonifico usando i dati qui sotto. Dopo aver effettuato il pagamento, clicca sul pulsante {"\u201C"}HO EFFETTUATO PAGAMENTO{"\u201D"}.
                    </p>
                    <div className="booking-bank-transfer-grid" role="group" aria-label="Dati bonifico">
                      <div className="booking-bank-transfer-item">
                        <span>Intestatario</span>
                        <strong>{BANK_TRANSFER_DETAILS.beneficiary}</strong>
                      </div>
                      <div className="booking-bank-transfer-item">
                        <span>IBAN</span>
                        <strong className="booking-bank-transfer-iban">{BANK_TRANSFER_DETAILS.iban}</strong>
                      </div>
                      <div className="booking-bank-transfer-item">
                        <span>Importo</span>
                    ...
                    {bankTransferAcknowledged ? (
                      <p className="booking-bank-transfer-feedback" role="status" aria-live="polite">
                        Grazie! Abbiamo ricevuto la tua segnalazione di pagamento e verificheremo il bonifico il prima possibile.
                      </p>
                    ) : null}
                  </div>
                ) : null}
              ...
              <div
                ...
                className={`booking-nav-row ${selectedPaymentProvider === 'paypal' ? 'booking-nav-row-paypal' : ''} ${
                  selectedPaymentProvider === BANK_TRANSFER_PROVIDER ? 'booking-nav-row-single' : ''
                }`}
              >
                ...
                ) : selectedPaymentProvider === 'mock' ? (
                  <div className="booking-bank-transfer-nav-placeholder" aria-hidden="true" />
                ) : selectedPaymentProvider === BANK_TRANSFER_PROVIDER ? (
                  <div className="booking-bank-transfer-nav-placeholder" aria-hidden="true" />
                ) : (
                  <button
          ...
          {currentStep === 7 ? (
            <div className="booking-stage-panel booking-stage-panel-success">
              <div className="booking-confirm-card booking-success-card">
                ...
                <span className="booking-success-pill">
                  {isBankTransferConfirmation ? 'Segnalazione pagamento ricevuta' : 'Pagamento completato'}
                </span>
                <h3 className="booking-strong-title">
                ...
                <p>
                  {isBankTransferConfirmation
                    ? 'La tua segnalazione e stata registrata. Ora contattaci subito per completare obbligatoriamente gli ultimi dettagli.'
                    : 'Il tuo tour e stato prenotato con successo. Abbiamo bloccato lo slot selezionato.'}
                </p>

                {isBankTransferConfirmation ? (
                  <div className="booking-bank-transfer-contact-box" role="status" aria-live="polite">
                    <p className="booking-bank-transfer-contact-title">Contatto obbligatorio per ultimi dettagli</p>
                    <p className="booking-bank-transfer-contact-copy">
                      Contattaci subito tramite uno dei canali seguenti per gli ultimi dettagli.
                    </p>
                    <div className="booking-bank-transfer-contact-actions">
                      <a href={SUPPORT_WHATSAPP_LINK} target="_blank" rel="noreferrer" className="booking-bank-transfer-contact-link">
                        WhatsApp
                      </a>
                      <a href={`mailto:${SUPPORT_EMAIL}`} className="booking-bank-transfer-contact-link">
                        {SUPPORT_EMAIL}
                      </a>
                      <a href={SUPPORT_PHONE_LINK} className="booking-bank-transfer-contact-link">
                        {SUPPORT_PHONE_LABEL}
                      </a>
                    </div>
                  </div>
                ...
                <div className="booking-confirm-row">
                  ...
                  <div>
                    ...
                    <strong>{completedBooking?.paymentReference || (isBankTransferConfirmation ? 'SEGNALAZIONE INVIATA' : '-')}</strong>
                    <small>
                      Metodo: {completedBooking?.paymentProvider || (selectedPaymentProvider === BANK_TRANSFER_PROVIDER ? 'BONIFICO' : selectedPaymentProvider.toUpperCase())} | Conferma: {completedBooking?.confirmedAt || (isBankTransferConfirmation ? 'In attesa verifica' : '-')}
                    </small>
                  </div>
                </div>
                ...
                <div className="booking-confirm-row">
                  <div className="icon">{"\u20AC"}</div>
                  <div>
                    <small>{isBankTransferConfirmation ? 'Totale Prenotazione' : 'Totale Pagato'}</small>
                    <strong>EUR {formatEurAmount(completedBooking?.total ?? (selectedTour ? selectedTour.price * Number(people) : 0), locale)}</strong>
                  </div>
                </div>
              </div>
