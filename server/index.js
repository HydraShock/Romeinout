...
const paymentProvidersByMode = {
  ...
  postepay: ['bank_transfer'],
  hybrid: ['paypal', 'bank_transfer'],
};
...
app.get('/api/payment-config', (req, res) => {
  res.json({
    ...
    bankTransfer: {
      enabled: allowedPaymentProviders.includes('bank_transfer'),
      beneficiary: bankTransferBeneficiary,
      iban: bankTransferIban,
      notes: bankTransferNotes,
    },
...
app.post('/api/admin/appointments/:id/confirm', requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    res.status(409).json({ message: 'La prenotazione non e in attesa di conferma.' });
  } catch (error) {
    res.status(500).json({ message: 'Errore durante la conferma prenotazione admin.' });
  }
});
  ...
  try {
    const bookedResult = await pool.query(
      `
        ...
        FROM appointments
        WHERE booking_date >= $1 AND booking_date < $2 AND status IN ('confirmed', 'pending')
        GROUP BY booking_date, time_slot
      `,
      [from, to]
    );
...
app.post('/api/booking-intents', async (req, res) => {
  ...
  try {
    const countsResult = await pool.query(
      `
        SELECT
          (SELECT COUNT(*) FROM appointments WHERE booking_date = $1 AND time_slot = $2 AND status IN ('confirmed', 'pending')) AS booked,
...
app.post('/api/bookings/confirm', async (req, res) => {
  ...
  const requestedPaymentProvider = String(paymentProvider || '').trim().toLowerCase();
  const normalizedPaymentProvider = requestedPaymentProvider === 'postepay'
    ? 'bank_transfer'
  ...
  const normalizedPaymentReference = paymentReference
    ...
    : (
      normalizedPaymentProvider === 'mock' || normalizedPaymentProvider === 'bank_transfer'
  ...
  const appointmentStatus = normalizedPaymentProvider === 'bank_transfer' ? 'pending' : 'confirmed';
  ...
  try {
    ...
    const slotResult = await connection.query(
      `
        ...
        FROM appointments
        WHERE booking_date = $1 AND time_slot = $2 AND status IN ('confirmed', 'pending')
      `,
    ...
    res.status(201).json({
      ...
      message: appointmentStatus === 'pending'
        ? 'Prenotazione registrata in attesa di conferma admin.'
