...
const BentoCardGrid = ({ children, gridRef, className = '' }) => (
  <div className={`magic-bento-grid bento-section ${className}`.trim()} ref={gridRef}>
    {children}
  </div>
);
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef} className={className}>
        {cards.map((card, index) => {
          const baseClassName = `magic-bento-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} ${cardClassName} ${card.className || ''}`.trim();
          const cardProps = {
